...
| `i18n` | FAIL | Feature registered in `featureTranslations` (imports + entry). Missing → re-run `register-di.js` |
...
- `--persist-spec` writes the **sanitized** spec to `src/features/<Feature>/feature-spec.json`
  ...
  Grep it — it must contain no real secret values before committing.
- Delete `.claude-skill-tsc-baseline.json` and `.claude-skill-manifest.json` from the repo
  root (working files, not for commit). The manifest is also the rollback map: on abort,
  delete its `created` files and `git checkout --` its `patched` files.
...
- Navigation is manual: add an expo-router route file under `app/` to expose the screen.
