...
- [x] VERIFIED 2026-09-03 in a fresh `claude -p` session against the installed plugin: the body shows the absolute plugin-cache path and the inlined update check printed UP TO DATE. Original: Verify: invoke `/react-clean-architecture` in Claude Code, confirm the rendered line shows the absolute path, not the literal `${CLAUDE_SKILL_DIR}`. If it does NOT substitute inside a `>-` folded block or body text, keep the sentence but reword to "run `echo $CLAUDE_SKILL_DIR`".
    ...
    Stop:
      - hooks:
          - type: command
            ...
            timeout: 20
  ```
- [x] VERIFIED 2026-09-03 (fresh `claude -p`, installed plugin): frontmatter hooks fire on PreToolUse and Stop; `${CLAUDE_SKILL_DIR}` is NOT expanded in hook commands → 1.20.1 makes every command self-locating (plugin root → project → user). Re-verified on 1.20.1 (installed plugin, fresh `claude -p`): no hook errors on the allow path; on the block path (manifest + persisted spec + baseline file, final message not a question) the transcript records `hook_blocking_error Stop`, the model reads the reason, inspects, and asks the user — no loop. PreCompact/PostCompact VERIFIED 2026-09-03 via stream-json headless sessions sending `/compact`: they are NOT dispatched to skill-frontmatter hooks (compaction went through with no spec) → moved to plugin `hooks/hooks.json` in 1.20.2 and re-verified: refusal without a spec, resume context injected after a real compaction. Original: VERIFY GATE (must pass before merging 2.2): start a session, invoke the skill, run `/compact` with no manifest → no block; create an empty `.claude-skill-manifest.json` in a scratch repo, `/compact` → block message appears. If skill-frontmatter hooks do not fire for `PreCompact`/`PostCompact`/`Stop` (the guide only demonstrates `PreToolUse` there), move those three to Phase 3.3's plugin `hooks/hooks.json` and document a copy-paste `settings.json` block in the README for non-plugin installs. Evidence so far: the 2.1.259 binary registers frontmatter `hooks` for file-based skills (only MCP-sourced skills are refused) and knows PreCompact/PostCompact/stop_hook_active/CLAUDE_SKILL_DIR; the scripts themselves are unit-tested against piped stdin.
- [x] (happened: gate failed on this exact point; fixed in 1.20.1 with the self-locating command instead of the readlink form) If `${CLAUDE_SKILL_DIR}` is not expanded inside frontmatter `command:` strings, fall back to `"$CLAUDE_PROJECT_DIR"`-relative discovery: the hook scripts locate the skill dir via `path.join(__dirname, '..', '..')` anyway, so only the invoking path matters — use `node "$(dirname "$(readlink -f ~/.claude/skills/react-clean-architecture/SKILL.md)")/scripts/hooks/…"` as the documented fallback.
...
- [x] DONE in 1.20.2 (gate failed for the compaction pair only): `hooks/hooks.json` carries PreCompact/PostCompact; the three tool/stop hooks stay in the skill frontmatter. Original: `hooks/hooks.json` at the plugin root, same five hooks, commands as `node "${CLAUDE_PLUGIN_ROOT}/skills/react-clean-architecture/scripts/hooks/<name>.js"`
- [x] DONE in 1.20.2. Original: `README.md` — settings.json snippet for symlink/copy installs (same JSON with an absolute path)
