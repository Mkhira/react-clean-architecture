...
| `review-conventions` | FAIL | The conventions PR reviewers enforce by hand, in full in [REVIEW.md](REVIEW.md) (all from real review rounds): the feature directory is kebab-case; no dead pre-1.11.0 directories (`domain/usecases/`, `domain/IServices/`) — note the current interface layout `data/IServices/` + `domain/IRepositories/` is the owner's standing decision and is NOT flagged; the feature error type does not widen `AppError` (no `Omit<AppError,'code'>`, no invented `HTTP_ERROR`/`PARSE_ERROR`) and aliases `INFRA_ERROR_CODES` rather than deriving a union from a local `as const satisfies` array; mappers are exported functions (`to<Entity>(dto)` / `to<Action>RequestDTO(input)`), never a `<Action>Mapper = {}` object; no enum literal array repeated across files (move it to `domain/constants/`); no raw numbers or RN keyword strings in a `styles.ts` (use theme tokens, adding one to `baseStyles.ts` + the `Theme` type if it is missing); no presentation module that nothing imports. Fix the code — never silence the check |
| `mock-committable` | FAIL | `spec.mock` features only: `<Feature>MockService.ts` is git-ignored while the COMMITTED `src/core/di/container.ts` imports it — the app compiles on the author's machine and nowhere else (live finding 2026-08-23: EstablishmentSignup's container import resolved to a file no clone had, so `tsc` failed and DI threw at launch). Fix is a repo-policy call — un-ignore the mock service, or keep the mock registration out of `container.ts` — so ASK the owner; never edit `.gitignore` yourself. WARN instead when the file is ignored but nothing committed references it |
| `components-md` | WARN | COMPONENTS.md drift: a `src/shared/components` component with no dictionary entry (DRIFT) or an entry matching no component (STALE). **WARN so it never blocks a run — but you MUST still write the missing entries before reporting the run finished**; an unlisted component is invisible to the reuse gate and gets rebuilt by hand next time (that is exactly how `ContactOtpSheet` was duplicated). Procedure and entry format: DESIGN.md → "Keeping COMPONENTS.md current"; `node <skill>/scripts/check-components-md.js` for the list. Read the dictionary itself with `node <skill>/scripts/components.js` (index) / `components.js <Name>...` (entries) — never a whole-file read |
